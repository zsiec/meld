...
func TestRepairForJointTailNPath(t *testing.T) {
	...
	if r := repairForJointTailN(k, []int{1_000_000, 0, 0, 0}, delta); r != 0 {
	...
	rIndep := repairForJointTailN(k, indep, delta)
	rCorr := repairForJointTailN(k, corr, delta)
	...
	if rH := repairForJointTailN(k, heavier, delta); rH < rCorr {
	...
	if r := repairForJointTailN(k, []int{656_100, 291_600, 48_600, 3_600, 100}, delta); r <= 0 {
	...
	if got, want := repairForJointTail(k, pa, pb, pBoth, delta),
		repairForJointTailN(k, []int{1_000_000 - pa - pb + pBoth, pa + pb - 2*pBoth, pBoth}, delta); got != want {
